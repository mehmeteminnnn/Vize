...
test_that("Test : Latitude adl?? s??tun numeric de??erlerden olu??mal??d??r.", {
  expect_is(maps$Latitude, "numeric", info = "Latitude s??tunu numeric de??erlerden olu??muyor.")
})
test_that("Test : idx adl?? de??i??ken Global Workspace???de mevcuttur.", {
  expect_true(exists("idx", envir = .GlobalEnv), info = "idx adl?? de??i??ken mevcut de??il.")
})

test_that("Test : idx nesnesinin tipi (class'??) integer'dir.", {
  expect_is(idx, "integer", info = "idx nesnesinin tipi integer de??il.")
})
test_that("Test : Year adl?? s??tun numeric de??erlerden olu??mal??d??r.", {
  expect_is(maps$Year, "numeric", info = "Year s??tunu numeric de??erlerden olu??muyor.")
})